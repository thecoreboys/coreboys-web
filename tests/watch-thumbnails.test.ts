...
test("rail artwork avoids oversized and often missing YouTube stills", () => {
  ...
  assert.equal(chain.at(-1), "/embed-preview.png");
...
test("custom posters and provider images fall back to local artwork", () => {
  const custom = watchThumbCandidates("/brand/custom.webp", "video123");
  assert.equal(custom[0], "/brand/custom.webp");
  assert.deepEqual(watchThumbCandidates("https://cdn.example/image.jpg"), ["https://cdn.example/image.jpg", "/embed-preview.png"]);
  assert.deepEqual(watchThumbCandidates(""), ["/embed-preview.png"]);
});
