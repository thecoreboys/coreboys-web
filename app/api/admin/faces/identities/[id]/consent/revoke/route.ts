...
import { updateFaceIdentity } from "@/lib/face-recognition-store";
...
const Body = z.object({ reason: z.string().trim().min(10).max(500) }).strict();
...
export async function POST(request: Request, { params }: Context) {
  ...
  if (!z.string().uuid().safeParse(id).success || !body.success) {
    return facePrivateJson({ error: "invalid consent revocation" }, 400);
  }
  try {
    const result = await updateFaceIdentity(id, {
      action: "revoke_consent",
      reason: body.data.reason,
    }, { actor: auth, requestId: faceRequestId(request) });
