...
import { FaceTrackActionSchema } from "@/lib/face-recognition-contracts";
import {
  facePrivateJson,
  facePrivateResponse,
  faceRequestId,
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  const body = FaceTrackActionSchema.safeParse(await request.json().catch(() => null));
  if (!z.string().uuid().safeParse(id).success || !body.success) return facePrivateJson({ error: "invalid track review action" }, 400);
  try {
    const result = await reviewFaceTrack(id, body.data, { actor: auth, requestId: faceRequestId(request) });
