...
- Face-retention credential created in GitHub without exposing its value; next deploy installs the corresponding runtime secret. Media migration target now matches the runtime dedicated/primary database selection. Production verification pending.
- Deep multimodal analysis, expanded Originals automation and the remaining player/Guide phases are not yet delivered. Existing third-party source policies remain metadata-only; permission to process their binaries has not been supplied.
