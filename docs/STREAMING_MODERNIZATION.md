...
### Caption pilot verification — September 9 UTC

- Release 5211cd2 deployed successfully as Azure revision 0000063, with CI/security checks passing. Production mobile settings now keeps cards and descriptions within the viewport; the settings-category strip scrolls separately.
- Full web suite: 806 tests passing, typecheck passing, real PostgreSQL transcript lifecycle passing. Six offline Python contract checks cover channel restrictions, caption-only acquisition, public/non-live sources, language selection and sanitized provider failures.
- Local end-to-end acquisition imported two official CORE videos into the isolated development database: 4,589 caption cues, 318 windows, both private drafts. Production acquisition through the working local connection subsequently imported two different CORE videos: 3,318 cues, 221 windows, both private drafts. No paid AI calls or video downloads were made.
- Hosted run 34301885230 established the exact blocker: YouTube returns `provider_verification_required` from GitHub-hosted runners. This is not a database/configuration failure and must not be hidden as success or bypassed with cookies/proxies. Only `cron-core-captions` is disabled manually until a supported unattended source is established. Existing media-intelligence/social schedules remain enabled; do not claim daily caption refresh is operational.
- Scripts remain available for operator-authorized manual execution on a working connection. Use Node with `--conditions=react-server --import tsx`; `server-only` is now an explicit dependency. Supply a Python environment with pinned `yt-dlp==2026.8.19`, `CORE_CAPTION_PROCESSING_AUTHORIZED=true`, and the recorded permission reference. Omit `--apply` for a two-candidate read-only dry run. Never print the database credential or provider response body. Review drafts at `/admin/media-intelligence` before publishing evidence.
- Remaining work includes a supported unattended caption connection, expanded rights-cleared source coverage, actual paid/budgeted scene/transcription/embedding adapters, and the rest of the provider/product acceptance matrix. This milestone is not completion of all requested phases.
