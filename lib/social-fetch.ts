...
type Platform = "tiktok" | "instagram" | "x" | "twitter" | "youtube";

/** Normalised response — only the field the UI actually shows. */
export type SocialCount = number;

function endpointFor(platform: Platform, handle: string, url?: string): string | null {
...
function pickCount(platform: Platform, body: unknown): number | null {
