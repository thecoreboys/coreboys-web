...
export function PassportPresenceBridge() {
  ...
  return (
    <aside className={styles.toast} aria-live="polite" aria-label="Passport reward available">
      ...
      <div className={styles.copy}><span>{claimOutcome === "card" ? "Record added" : claimOutcome === "attendance" ? "Attendance verified" : pendingCertification ? "Attendance recorded" : "Record ready"}</span><strong>{eligible.event.title}</strong><small>{claimOutcome === "card" ? "It is now in your Memory Book." : claimOutcome === "attendance" ? "Your verified attendance is saved; no event record is attached yet." : pendingCertification ? "An independent operator must certify the event before permanent rewards can be claimed." : "Your verified live presence is ready to add to your Passport."}</small></div>
