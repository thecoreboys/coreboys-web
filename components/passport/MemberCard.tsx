...
export function MemberCard({ passport }: { passport: PassportDashboard }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-primary bg-primary p-4 sm:p-5" aria-label="Your CORE Passport identity">
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-tertiary">CORE Passport</p><h2 className="mt-1 text-lg font-semibold text-primary">Your account identity</h2><p className="mt-1 text-xs text-tertiary">These values come from verified activity recorded on your account.</p></div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-center">
        <article className={`relative aspect-[1.58/1] overflow-hidden rounded-2xl ${cardClass} p-4 shadow-sm ring-1 ring-black/10`} style={{ ["--member-card-accent" as string]: style.accent }}>
        ...
        {canCustomize ? (
          ...
          <Link href={subscription.featureHref("passport.card_customization") as never} className="flex min-h-32 items-center gap-3 rounded-2xl bg-secondary p-4 text-left ring-1 ring-primary transition hover:bg-tertiary"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary text-primary"><LockKeyhole className="size-4" /></span><span><span className="block text-xs font-semibold text-primary">Appearance settings</span><span className="mt-1 block text-[10px] leading-4 text-tertiary">Membership adds more profile appearance options. Your recorded stats stay unchanged.</span></span></Link>
        )}
      </div>
    </section>
  );
