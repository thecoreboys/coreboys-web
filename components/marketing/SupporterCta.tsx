...
export function SupporterCta({ placement = "watch", compact = false, className = "" }: SupporterCtaProps) {
  ...
  if (placement === "watch" && !compact) {
    return <SupporterDiscoveryGrid className={className} />;
  }
  return (
...
function SupporterDiscoveryGrid({ className }: { className: string }) {
  return (
    <section className={`supporter-discovery-grid ${className}`} aria-label="Support the site membership benefits">
      <article className="member-hero member-hero--single relative isolate min-h-[24rem] overflow-hidden rounded-2xl bg-[#0b0a0f] p-5 text-white sm:aspect-[1933/814] sm:min-h-0 sm:p-7" aria-label="Support the site">
        <img src="/brand/supporter/supporter-vault-hero-v3.png" alt="" aria-hidden className="member-hero__generated-art absolute inset-0 -z-20 h-full w-full object-cover object-center" />
        ...
        <div className="relative z-10 flex min-h-[20rem] flex-col items-center justify-center py-8 text-center sm:h-full sm:min-h-0">
          <div className="member-hero__offer flex max-w-[34rem] flex-col items-center">
            <p className="member-hero__price inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-bold uppercase tracking-[.14em]"><Sparkles className="size-3.5" aria-hidden /> Starting from $3/month</p>
            ...
            <Link href="/account/plan" className="member-hero__action mt-7 inline-flex min-h-12 items-center gap-2.5 rounded-xl px-5 text-sm font-extrabold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"><span className="member-hero__action-label">Support the site</span><span className="rounded-md px-2 py-1 text-[11px]">$3/month</span><ArrowRight className="size-4" aria-hidden /></Link>
