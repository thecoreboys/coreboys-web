...
function NotificationPreview({ item, onRead }: { item: InboxNotification; onRead: (id: string) => void }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group flex gap-3 rounded-xl px-3 py-3 text-left transition hover:bg-[color:var(--surface)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--core)]",
        !item.readAt && "bg-[color:var(--bg-elev)]",
      )}
    >
    ...
    <div ref={containerRef} className="relative">
      ...
      {open ? (
        ...
        >
          ...
          <div className="max-h-[min(27rem,calc(100dvh-8rem))] overflow-y-auto p-1.5">
            ...
            {data?.items.map((item) => <NotificationPreview key={item.id} item={item} onRead={(id) => void markRead(id)} />)}
          </div>
