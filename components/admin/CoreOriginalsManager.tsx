...
export function CoreOriginalsManager() {
  ...
  return <div className="space-y-8">
    ...
    <section className={card}><h2 className="text-lg font-semibold text-primary">Create a CORE Original</h2><p className="mt-1 text-sm text-tertiary">Each original gets a public collection page and a private approval queue.</p>
      <form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const value = new FormData(form); void request("POST", { action: "create-original", slug: value.get("slug"), title: value.get("title"), summary: value.get("summary") || null, posterUrl: value.get("posterUrl"), sortOrder: Number(value.get("sortOrder") || 100), enabled: true }).then((ok) => { if (ok) form.reset(); }); }}>
    ...
    <section className={card}><label className="text-sm font-semibold text-primary">Manage an original<select value={active?.id ?? ""} onChange={(event) => setSelected(event.target.value)} className={`${input} mt-2`}>{data.originals.map((entry) => <option key={entry.id} value={entry.id}>{entry.title}</option>)}</select></label>
      {active ? <div className="mt-5 space-y-6">
        ...
        <AddItem key={active.id} originalId={active.id} request={request} busy={busy} />
...
function AddItem({ originalId, request, busy }: { originalId: string; request: (method: "GET" | "POST" | "PATCH", body?: unknown) => Promise<boolean>; busy: boolean }) {
  return <form className="border-t border-secondary pt-5" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const value = new FormData(form); void request("POST", { action: "add-item", originalId, sourceUrl: value.get("sourceUrl"), platform: value.get("platform"), title: value.get("title"), subtitle: value.get("subtitle") || null, posterUrl: value.get("posterUrl") || null, format: value.get("format"), sortOrder: Number(value.get("sortOrder") || 100), status: "approved" }).then((ok) => { if (ok) form.reset(); }); }}><h3 className="font-semibold text-primary">Add an approved item</h3><p className="mt-1 text-sm text-tertiary">Use the order number to arrange episodes or entries within this series.</p><div className="mt-3 grid gap-3 md:grid-cols-2"><input className={input} name="title" required placeholder="Title" /><input className={input} name="sourceUrl" type="url" required placeholder="Public video URL" /><select className={input} name="platform"><option value="youtube">YouTube</option><option value="tiktok">TikTok</option><option value="instagram">Instagram</option><option value="twitch">Twitch</option><option value="x">X</option><option value="other">Other</option></select><select className={input} name="format"><option value="auto">Auto</option><option value="long">Video</option><option value="short">Short</option><option value="photo">Photo</option></select><input className={input} name="sortOrder" type="number" min="0" defaultValue="100" placeholder="Series order" /><input className={input} name="posterUrl" type="url" placeholder="Poster URL (optional)" /><input className={input} name="subtitle" placeholder="Subtitle (optional)" /></div><button className={`${action} mt-3`} disabled={busy}>Add to public collection</button></form>;
}
